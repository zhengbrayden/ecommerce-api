  ...
  while (true) {
    try {
        await dbSession.withTransaction(async () => {
            ...
            let transaction = await Transaction.findOne({stripeSessionId: sessionId}).session(dbSession)
        ...
        if (isTransient && retries < RETRY_LIMIT) {
            console.warn(`TransientTransactionError, retrying transaction (attempt ${retries + 1})...`);
            retries++;
